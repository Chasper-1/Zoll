...
//
// Диспетчеризация на стороне движка: match по своему enum (джамп-таблица),
// редактор получает уже типизированные вызовы.
pub(crate) fn dispatch_spans(sink: &mut dyn SpanSink, revision: u64, spans: &[SyntaxSpan]) {
